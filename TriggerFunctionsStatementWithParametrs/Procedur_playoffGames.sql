create or replace procedure PlayoffSeries(homeTeam in String, hostedTeam in String)
is
--fields utils

zero_found integer = 0;
...
number_of_arenas integer = select Count(*) from (select distinct ArenaName from Game);

number_of_refrees integer = select Count(*) from (select distinct personid from Refree);

half_standing_number integer = select Floor((Count(*) / 2)) from (select distinct TeamName from Standing);
